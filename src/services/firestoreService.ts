  ...
  const workbookData = {
    ...
    data: {
      ...
      day2: {
        ...
        launchStrategy: "",
        migration: "",
        firstClients: Array.from({ length: 10 }, () => ({ name: "", reason: "" })),
      },
...
export const calculateCompletionPercentage = (data: any): number => {
  const fields = [
    ...
    data.day1?.modelName,
    data.day1?.avatarDescription,
    data.day1?.consciousnessLevel,
    data.day1?.clientPhrases,
    data.day1?.transformation,
    data.day1?.formula,
    data.day1?.modelType,
    data.day1?.modelReason,
    data.day1?.support,
    data.day1?.content,
    data.day1?.community,
    data.day1?.progress,
